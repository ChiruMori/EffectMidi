...
export default {
  ...
  waiting: (_: any): number[] => [CMD_BEGIN_BYTE, CMD_BYTE_WAITING],
  ...
  keyDown: (key: number): number[] => [CMD_BEGIN_BYTE, CMD_BYTE_KEY_DOWN, key],
  keyUp: (key: number): number[] => [CMD_BEGIN_BYTE, CMD_BYTE_KEY_UP, key],
  setBrightness: (brightness: number): number[] => [
    CMD_BEGIN_BYTE,
    CMD_BYTE_SET_BRIGHTNESS,
    brightness
  ],
  setResidualTime: (residualTime: number): number[] => [
    CMD_BEGIN_BYTE,
    CMD_BYTE_SET_RESIDUAL_TIME,
    residualTime
  ],
  ...
  colorPreview: (colorHex: string): number[] => [
    CMD_BEGIN_BYTE,
    CMD_BYTE_COLOR_PREVIEW,
    ...parseColor(colorHex)
  ]
