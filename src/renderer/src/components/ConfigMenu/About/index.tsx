...
export default function About({ hidden }: { hidden: boolean }): JSX.Element {
  const stat = useAppSelector(statisticsSelector)
  ...
  return (
    ...
    >
      ...
      <p className="inline">
        ...
        <a
          onClick={(e) => openLink(e, pkg.homepage)}
          className={`inline-flex items-center cursor-pointer underline text-${theme}-sub`}
        >
          <span>Github</span>
          <span>
      ...
      <p className="mt-4">
        {txt('about.stat-score')}: <span className={`text-${theme}-sub`}>{stat.score ?? 0}</span>
      </p>
      <p>
        {txt('about.stat-paddle')}: <span className={`text-${theme}-sub`}>{stat.paddle ?? 0}</span>
      </p>
    </div>
  )
}
